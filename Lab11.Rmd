...
p + coord_flip() +stat_summary(fun.y=mean, geom="point", shape=5, size=4)  #flipping x and y coordinates for better readability
...
m5<-ggplot(data=d, mapping = aes(x= Sepal.Width, color=Species )) + geom_bar()
...
m6<-ggplot(data=d2, aes(x=Species, fill=Species)) + geom_bar() + labs(title="Barplot of Sepal Width",
       y="Count") + ylim(0,50)
...
m7 <- ggplot(data=d3, aes(x=Species, y=value, fill=variable)) + geom_bar(stat= "identity", position = "dodge") #stat tells R to group by the different measurements, and position= dodge tells R to place each measurement per species into a separate bar
print(m7)
